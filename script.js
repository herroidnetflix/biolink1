...
(() => {
  ...
  const setButton = (playing) => {
    ...
    toggle.setAttribute(
      "aria-label",
      playing ? "Turn background music off" : "Turn background music on"
    );
  ...
  // Start after the first meaningful visitor interaction.
  ...
  interactionEvents.forEach((event) =>
    document.addEventListener(event, onInteraction, { capture: true, once: true })
  );
